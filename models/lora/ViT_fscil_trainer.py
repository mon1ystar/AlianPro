...
from models.lora.ViT_prompt_deep_orc import ViT_DEEP_ORC_A
...
class ViT_FSCILTrainer(Trainer):
    ...
    def get_optimizer_base(self):
        ...
        optimizer = torch.optim.Adam([
                {'params': self.model.module.encoder.parameters(), 'lr': 0.0001},
                {'params': self.model.module.fc.parameters(), 'lr': 0.1}
            ])
            ...
            if session == 0:  # load base class train img label
                for name, param in self.model.module.encoder.named_parameters():
                    if param.requires_grad:
                        print("baseTrainable:", name, param.shape)
                ...
                # self.model.module.train_inc_meanfeature(trainloader, self.args.epochs_new, session, np.unique(train_set.targets), args)
