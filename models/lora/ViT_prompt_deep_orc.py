...
class LoRALinear(nn.Module):
    def __init__(self, orig_layer: nn.Linear, r=8, lora_alpha=16, dropout=0.):
        super().__init__()
        self.orig = orig_layer  # 原始 nn.Linear 层
        self.r = r
        self.lora_alpha = lora_alpha
        self.dropout = nn.Dropout(dropout)
        self.lora_A = nn.Parameter(torch.randn(r, orig_layer.in_features) * 0.01)
        self.lora_B = nn.Parameter(torch.randn(orig_layer.out_features, r) * 0.01)
    ...
    def forward(self, x):
        ...
        lora_out = lora_out @ self.lora_B.T         # [B, *, out_dim]
        return out + self.scaling * lora_out

class LoRAQKV(nn.Module):
    def __init__(self, orig_qkv: nn.Linear, r=8, alpha=16):
        super().__init__()
        self.qkv = LoRALinear(orig_qkv, r=r, lora_alpha=alpha)
...
def inject_lora_into_vit(model, r=8, alpha=16):
    for i, block in enumerate(model.blocks):
        qkv = block.attn.qkv
        block.attn.qkv = LoRAQKV(qkv, r=r, alpha=alpha)
...
class ViT_DEEP_ORC_A(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        inject_lora_into_vit(self.encoder, r=8, alpha=16)
    ...
    def inject_prompt_tokens(self,patch_tokens, prompt_tokens, indices):
        
        patch_tokens = patch_tokens.clone()
        prompt_tokens = prompt_tokens.unsqueeze(0).expand(patch_tokens.shape[0], -1, -1)
        for i, idx in enumerate(indices):
            patch_tokens[:, idx, :] = prompt_tokens[:, i, :]
        return patch_tokens
    ...
    def forward(self, x):
        ...
        cls_token = self.ft_encode(x)

        logits = torch.matmul(F.normalize(cls_token, dim=1), F.normalize(self.fc.weight, dim=1).t())
    ...
    def train_inc(self, dataloader, epochs, session, class_list,args):
        ...
        optim = torch.optim.Adam([
                ...
                {'params': self.fc.parameters(), 'lr': 0.1}
            ])
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optim, T_max=epochs)
        for epoch in range(epochs):
            for batch in dataloader:
                data_imgs, label = [_.cuda() for _ in batch]
                logits = self.forward(data_imgs) 
                ...
                acc = (pred == label).sum().item()/label.shape[0]*100.
                print(f"[{epoch}/{epochs}] Loss_CE:{loss.item():.4f} ACC: {acc}")

    def train_inc_meanfeature(self, dataloader, epochs, session, class_list,args):
        print("[Session: {}]".format(session))
        for batch in dataloader:
            data_imgs, label = [_.cuda() for _ in batch]
            x = self.encoder.patch_embed(data_imgs)
            cls_token = self.ft_encode(x)
            cls_embed=cls_token.detach()
    ...
    def update_fc_avg(self,dataloader,class_list,session):
        ...
        with torch.no_grad():
            ...
            for class_index in class_list:
                ...
                # self.fc.classifiers[0].weight.data[class_index] =   0.1 * base_vector + self.fc.classifiers[0].weight.data[class_index]
                # self.fc.classifiers[0].weight.data[class_index] = F.normalize(self.fc.classifiers[0].weight.data[class_index], p=2, dim=0)
            average_max_cos = sum(max_cos_list) / len(max_cos_list)
            print(average_max_cos)
        self.train()
    
    def update_hc_weightmean(self,dataloader,class_list,session):
        self.eval()
        ...
        with torch.no_grad():
            ...
            for class_index in class_list:
                data_index=(label==class_index).nonzero().squeeze(-1)
                hc_samples = hidden_embed[data_index]
                base_protos = torch.stack(self.hc)  # shape: (N_old, d)
                # 计算当前样本与所有旧类原型的余弦相似度
                cos_sim = F.cosine_similarity(
                    hc_samples.unsqueeze(1),   # (5, 1, d)
                    base_protos.unsqueeze(0),  # (1, N_old, d)
                    dim=-1                     # → (5, N_old)
                )
                # 对每个样本取与所有旧类相似度的最大值（或均值）
                max_sim = cos_sim.max(dim=1)[0]  # shape: (5, )

                # 转为“相异度”，可加个 margin 或 1 - sim
                weights = 1.0 - max_sim
                ...
                # 加权求原型
                hc = torch.sum(hc_samples * weights.unsqueeze(1), dim=0)
