...
from models.negative_prompt.ViT_prompt_deep_orc import ViT_DEEP_ORC_A
    ...
    def train(self):
        ...
        for session in range(args.start_session, args.sessions):
            ...
            if session == 0:  # load base class train img label
                ...
                if self.args.no5shot:
                    self.model.module.train_inc(trainloader, self.args.epochs_new, session, np.unique(train_set.targets), args)
                else:
                    self.model.module.train_inc_5shot(trainloader, self.args.epochs_new, session, np.unique(train_set.targets), args)
                self.model.eval()
                self.model.module.mode = 'avg_cos'
                tsl, tsa, logs = test(self.model, testloader, 0, args, session,self.word_info)
