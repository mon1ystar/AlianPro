...
def base_train(model, trainloader, optimizer, scheduler, epoch, word_info, query_info, class_list, args, loss_curve):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        v_label = map_range(train_label, 0, args.base_class-1, args.base_class, args.num_classes-1).long()
        logits, cls_embedding,hidden_embedding,vlogits,vembedding= model(data)
        loss_angle =  myloss(logits, train_label)
        loss_v = myloss(vlogits, v_label)
        
        
        
        acc = count_acc(logits, train_label)
        total_loss = loss_angle + 0.001*loss_v
...
def test(model, testloader, epoch, args, session, word_info):
    ...
    with torch.no_grad():
        ...
        if session == 10:
            ...
            pred_labels = torch.cat(all_pred_labels).cpu().numpy()

            # 生成混淆矩阵并保存数据
            cm = confusion_matrix(true_labels, pred_labels, labels=np.arange(test_class))
            np.save(f'exp_fig/finutune_confusion_matrix_session{session}.npy', cm)  # 保存混淆矩阵

            # 设置字体大小
            plt.figure(figsize=(12, 10))
            sns.set(font_scale=1.4)  # 增大热图上的文字大小

            # 使用高对比度配色，例如 'magma' 或 'rocket'
            sns.heatmap(cm, cmap='magma', xticklabels=False, yticklabels=False, cbar=True)

            plt.title(f'Confusion Matrix - Session {session}', fontsize=18)
            plt.xlabel('Predicted Labels', fontsize=16)
            plt.ylabel('True Labels', fontsize=16)
            plt.tight_layout()

            # 保存图片
            plt.savefig(f'exp_fig/finetune_confusion_matrix_session{session}.png', dpi=300)
            plt.close()
