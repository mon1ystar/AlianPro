...
def inject_adapter(block):
    def forward(x):
        x = x + block.attn(block.norm1(x))
        x = x + block.mlp(block.norm2(x))
        x = block.adapter(x)  # adapter 加在 block 之后
        return x
    return forward
...
class ViT_DEEP_ORC_A(nn.Module):
    ...
    def __init__(self, args, mode=None):
        ...
        for blk in self.encoder.blocks:
            ...
            blk.adapter = Adapter(dim=dim)  # 添加 adapter
            blk.forward = inject_adapter(blk)  # 替换 forward 函数
    ...
    def forward(self, x):
        ...
        # x_mask = self.inject_prompt_tokens(x, self.mask_prompt, random.sample(range(196), 5)) 

        # hidden_embedding,vcls = self.prompt_encode(x,)
        # hidden_embedding_mask,vcls = self.prompt_encode(x_mask,)
        hidden_embedding,vcls = self.ft_encode(x,)
        # hidden_embedding = self.encoder.forward_features(x)[:,0]
        vcls_mean = vcls.mean(dim=1)
        cls_embedding = self.projector(hidden_embedding)
        mask_embedding = self.projector(vcls_mean)
        logits = self.fc.get_logits(cls_embedding)
        vlogits = self.fc.get_logits(mask_embedding)
        ...
        return logits, cls_embedding,hidden_embedding,vlogits,mask_embedding,
    ...
    def train_inc(self, dataloader, epochs, session, class_list,args):
        ...
        # self.update_hc_weightmean(dataloader, class_list, session)#计算hc和新类原型
        self.update_fc_avg(dataloader, class_list, session)
    ...
    def train_inc_5shot(self, dataloader, epochs, session, class_list,args):
        ...
        for epoch in range(epochs):
            ...
            old_label = torch.arange(num_base).cuda()
