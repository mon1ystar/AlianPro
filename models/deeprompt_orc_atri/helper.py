...
def base_train(model, trainloader, optimizer, scheduler, epoch, word_info, query_info, class_list, args, loss_curve):
    ...
    for i, batch in enumerate(tqdm_gen, 1):
        ...
        loss_sup = 0.01 * supervised_contrastive_loss(hidden_embedding,train_label)
        
        # mixup
        mixup_data,mixup_label = mixup(data, train_label)
        mix_logits, _,_,vlogits,vembedding= model(mixup_data)
        loss_mixup = myloss(mix_logits, mixup_label)

        
        
        # 虚拟提示的损失 和 提示间正交
        loss_v = logits_to_zero_loss(vlogits)
        loss_orth = orthogonality_loss(vembedding)
        
        acc = count_acc(logits, train_label)
        total_loss = loss_angle + 0.001 * loss_v + 0.1 * loss_orth + 0.01 * loss_mixup
        # total_loss = loss_angle +  0.01 * loss_mixup
